<script lang="ts">
	...
	function setModeError(modeKey: ToolMode, message: string, statusMessage: string) {
		updateModeResult(modeKey, {
		...
		if (shadowIsColored) {
			...
			for (let index = 0; index < innerGray.length; index += 1) {
				alphaCache[index] = Math.min(
					Math.max((255 + innerGray[index] * innerScale - (255 - (255 - coverGray[index]) * coverScale)) / 255, 0),
					1,
				);
			}
			...
			for (let index = 0; index < innerPixels.length; index += 4) {
				...
				outputData[index] = clamp(
					((innerCache[index] - alphaColor + 255 - coverCache[index]) * shadowInnerWeight + alphaColor - 255 + coverCache[index]) / safeAlpha,
					0,
					255,
				);
				outputData[index + 1] = clamp(
					((innerCache[index + 1] - alphaColor + 255 - coverCache[index + 1]) * shadowInnerWeight + alphaColor - 255 + coverCache[index + 1]) / safeAlpha,
					0,
					255,
				);
				outputData[index + 2] = clamp(
					((innerCache[index + 2] - alphaColor + 255 - coverCache[index + 2]) * shadowInnerWeight + alphaColor - 255 + coverCache[index + 2]) / safeAlpha,
					0,
					255,
				);
